...
The third question will run a Monte Carlo simulation to test for independence between team role type and country of birth. Since Monte Carlo does not have underlying distribution assumptions, we can do a general contingency table with `length(unique(na.omit(x$team_role_type)))` columns characterized by the scale ratings and rows grouped by 'Australian' and 'Internationals'.
...
3.  **Test statistic**: T = t = \$\frac{\bar{X}_1 - \bar{X}_2}{S_p \sqrt{\frac{1}{n_1} + \frac{1}{n_2}}}\] where \[S_p = \sqrt{\frac{(n_1 - 1)S_1^2 + (n_2 - 1)S_2^2}{n_1 + n_2 - 2}}\]Here, ( S_1\^2 ) and ( S_2\^2 ) \$ are the sample variances of the two groups.
...
Good work! You've done quite well with the assignment and it's clear you've put a lot of effort in. In addition to the comments in the rubric, my general comments are:

Please check your submission after submitting into Canvas to make sure the citations show up right. You could've included more references. Missing citations are the R packages used and maybe some news article to substantiate the story context. 

Some good discussion and excellent overarching theme.

Data cleaning section could be more concise - for example distribution of students by place of birth and employment status could be made into one proportion plot.

Some raw tibble outputs, some issues with formatting and Latex particularly in 3.2, a couple of mislabels.

For test 2, the QQplot shows that internationals really do tail off at the lower end - not really that normal, though the boxplot was fine for assuming normality. - you should've stated this. Limitations not stated. Could have stated that normality is debatable given the outlier tail in the QQplot so to justify the limitation of the two sample t test result. 

For Monte Carlo test, Limitations not stated. One of the cells contains 0, could have collapsed the rating columns into broader rating range. 
